...
from components.contextual_embeddings import BertEncoder, RobertaEncoder
from utils.sentence_processing import *
from utils.logger import print_log, store_results
from utils.helper import save_checkpoint, bleu_scorer
...
class TransformerModel(nn.Module):
  ...
  #     cont_reps, _ = self.embedding1(ques)  # Get contextualized representations
  #     src = cont_reps.transpose(0,1)
  #     print(f"Ques: {ques}")
  #     print(f"src: {src}")
  #     # print(f"src_tokens: {src_tokens}")
  #     # print(f"Type of src: {type(src)}, Expected: Tensor")
  #     # print(f"Type of src_tokens: {type(src_tokens)}, Expected: Tensor or similar")
  ...
  def forward(self, ques, src, trg):
    ...
    if self.config['embedding'] == 'bert' or self.config['embedding'] == 'roberta':
        cont_reps, src_tokens = self.embedding1(ques)
        src = cont_reps.transpose(0, 1)
        src_pad_mask = self.make_len_mask(src_tokens.transpose(0, 1))
        src = self.pos_embedding1(src)
    else:
        # Process src normally for non-BERT embeddings
        src_pad_mask = self.make_len_mask(src)
        src = self.embedding1(src)  # Embed source tokens
        src = self.pos_embedding1(src)  # Apply positional encoding

    # Prepare target sequence
    trg_pad_mask = self.make_len_mask(trg)
    trg = self.embedding2(trg)  # Embed target tokens
    trg = self.pos_embedding2(trg)  # Apply positional encoding
